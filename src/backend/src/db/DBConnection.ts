...
/**
 * Fallback host/user/port when building URI from `MONGO_PASSWORD` (no full `MONGODB_URI`).
 * Prefer `MONGODB_URI` / `MONGO_URI` in `.env` for non-local clusters.
 */
export const MONGO_DEFAULT_HOST = "127.0.0.1";
 ...
 * Uses {@link MONGO_AUTH_USERLOGIN_DB} when set; otherwise the same database as {@link getUserLoginCollection}
 * ({@link resolveUserLoginDatabaseName}: `MONGO_USERLOGIN_DB`, `MONGO_DATABASE`, or `ClubMaster_DB`).
 * Previously sign-in always defaulted to `ClubMaster_DB` and ignored `MONGO_USERLOGIN_DB`, which caused
 * "Invalid credentials" when accounts lived only on the configured app database.
 */
export function resolveAuthLoginDatabaseName(explicit?: string): string {
  ...
  const authOnly = process.env.MONGO_AUTH_USERLOGIN_DB?.trim();
  if (authOnly) {
    return authOnly;
  }
  return resolveUserLoginDatabaseName();
}
