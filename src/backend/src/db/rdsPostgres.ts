...
export function getRdsPool(): pg.Pool {
  ...
  if (!poolSingleton) {
    ...
    poolSingleton.on("error", (err) => {
      console.error("[rds pool] Unexpected idle client error:", err);
    });
  }
  return poolSingleton;
}

/** Introspection for /api/health (pg.Pool exposes these in node-pg 8+). */
export function getRdsPoolStats(): {
  configured: boolean;
  ...
  if (!poolSingleton) {
    return { configured: true, poolInitialized: false };
  }
