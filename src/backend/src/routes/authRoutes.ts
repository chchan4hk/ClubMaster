...
export function createAuthRouter(): Router {
   ...
   * Requires MongoDB (`MONGODB_URI`, etc.). Credentials are verified only against the `userLogin`
   * collection via `getAuthUserLoginCollection()` / `resolveAuthLoginDatabaseName()` (typically
   * `ClubMaster_DB.userLogin`). CSV/JSON login is not used; unconfigured Mongo returns 503.
   */
