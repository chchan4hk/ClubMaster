...
async function loginWithContext(username, password, role, clubName) {
  ...
  if (data.token && data.user) {
    /** Prefer Mongo/API `usertype` so the dashboard matches `ClubMaster_DB.userLogin`, not only the form role. */
    const page =
      resolveDashboardPage(data.user) ||
      (role && FORM_ROLE_TO_PAGE[role]);
    if (!page) {
      throw new Error("Unknown user type — cannot open a dashboard.");
    }
    if (
